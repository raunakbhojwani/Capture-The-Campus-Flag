...
static char* selection_handle_get_text(int index, void *context) {
  ...
  snprintf(
    ...
    sizeof(pin_window->field_buffs[0]), "%X",
...
PinWindow* pin_window_create(PinWindowCallbacks callbacks, bool captured) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      if (captured) {
        text_layer_set_text(pin_window->main_text, "Capture ID Required");
      }
      else {
        text_layer_set_text(pin_window->main_text, "Code ID Required");
      }
      ...
      text_layer_set_text(pin_window->sub_text, "Enter the ID to continue");
      ...
      for (int i = 0; i < PIN_WINDOW_NUM_CELLS; i++) {
        selection_layer_set_cell_width(pin_window->selection, i, 29);
      }
