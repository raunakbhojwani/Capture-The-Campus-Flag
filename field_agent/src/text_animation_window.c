...
void text_animation_window_push(char* text_1, char* text_2) {
  snprintf(s_text[0], sizeof(s_text[0]), text_1);
  snprintf(s_text[1], sizeof(s_text[1]), text_2);
  s_current_text = 0;

  if(!s_window) {
    s_window = window_create();
    window_set_background_color(s_window, PBL_IF_COLOR_ELSE(GColorWhite, GColorDarkGray));
