...
static char msg[200];
static bool being_captured = false;
...
static void window_load(Window *window) {
  ...
  // s_icon_bitmap = gbitmap_create_with_resource(RESOURCE_ID_CAPTURE);
  // GRect bitmap_bounds = gbitmap_get_bounds(s_icon_bitmap);
  ...
  s_label_layer = text_layer_create(GRect(DIALOG_MESSAGE_WINDOW_MARGIN, DIALOG_MESSAGE_WINDOW_MARGIN, bounds.size.w - (2 * DIALOG_MESSAGE_WINDOW_MARGIN), bounds.size.h - (2* DIALOG_MESSAGE_WINDOW_MARGIN)));
  text_layer_set_text(s_label_layer, msg);
  text_layer_set_background_color(s_label_layer, GColorRed);
